...
export const directMapping = async (
  ...
  for (var i = 0; i < directArr.length; i++) {
    ...
    if ((directArr[i].val = directArr[i].val.toString()).length > 9) 
    finalStr = "NA";
    let n = ('000000000' + directArr[i].val).substr(-9).match(/^(\d{2})(\d{2})(\d{2})(\d{1})(\d{2})$/);
    if (!n) finalStr = "NA"; 
    let str = '';
    ...
    variableTovalueMap[directArr[i].jPath] = finalStr;
    }
