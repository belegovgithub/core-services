...
public class SmsProperties1 {
	 ...
	 * String> requestBody) { if (isPriorityEnabled) {
	 * requestBody.add(smsPriorityParameterName, getSmsPriority(priority)); } }
	 */

	/*
	 * private void populateAdditionalSmsParameters(MultiValueMap<String, String>
	 * map) { if (isExtraRequestParametersPresent()) {
	...
	/*
	 * @Value("${sms.priority.enabled}") private boolean isPriorityEnabled;
	 */
	...
	/*
	 * @Value("${sms.priority.param.name}") private String smsPriorityParameterName;
	 */
	...
	/*
	 * private String getProperty(String propKey) { return
	 * this.environment.getProperty(propKey, ""); }
	 */

	/*
	 * private boolean isExtraRequestParametersPresent() { return
	 * StringUtils.isNotBlank(getProperty(SMS_EXTRA_REQ_PARAMS)); }
	 * 
	 * private HashMap<String, String> getExtraRequestParameters() { String[]
	 * extraParameters =
	 * getProperty(SMS_EXTRA_REQ_PARAMS).split(KEY_VALUE_PAIR_DELIMITER); final
	 * HashMap<String, String> map = new HashMap<>(); if (extraParameters.length >
	 * 0) { for (String extraParm : extraParameters) { String[] paramNameValue =
	 * extraParm.split(KEY_VALUE_DELIMITER); map.put(paramNameValue[0],
	 * paramNameValue[1]); } } return map; }
	 */
