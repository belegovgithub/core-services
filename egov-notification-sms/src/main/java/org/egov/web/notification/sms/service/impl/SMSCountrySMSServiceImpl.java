 ...
 * 
 * import java.net.URI; import java.net.URL; import java.util.List; import
 * java.util.Map;
 * 
 ...
 * @Service
 * 
 * @ConditionalOnProperty(value = "sms.gateway.to.use", havingValue =
 * "SMS_COUNTRY")
 * 
 * @Slf4j public class SMSCountrySMSServiceImpl implements SMSService {
 * 
 ...
 * 
 * private void submitToExternalSmsService(Sms sms) { try { String baseURL =
 * smsProperties.getUrl(); MultiValueMap<String, String> params =
 * getRequest(sms).getBody(); URI uri = UriComponentsBuilder
 * .fromUriString(baseURL) .queryParams(params) .build().encode().toUri();
 * String response = restTemplate.postForObject(uri, "{}", String.class);
 * log.info("response: "+response); } catch (RestClientException e) {
 ...
 * bodyBuilder.getSmsRequestBody(sms); return new HttpEntity<>(requestBody,
 * getHttpHeaders()); }
 * 
