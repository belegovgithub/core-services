 ...
 * java.security.NoSuchAlgorithmException; import java.util.List; import
 * java.util.Map.Entry;
 * 
 * import javax.net.ssl.SSLContext;
 * 
 * import org.apache.http.conn.ssl.NoopHostnameVerifier; import
 * org.apache.http.conn.ssl.SSLConnectionSocketFactory; import
 * org.apache.http.conn.ssl.TrustStrategy; import
 * org.apache.http.impl.client.CloseableHttpClient; import
 * org.apache.http.impl.client.HttpClients; import
 * org.egov.web.notification.sms.config.SmsProperties1; import
 * org.egov.web.notification.sms.models.Sms; import
 * org.egov.web.notification.sms.service.SMSService; import org.slf4j.Logger;
 ...
 * matchIfMissing = true) public class ExternalSMSService implements SMSService
 * { private static final Logger LOGGER =
 * LoggerFactory.getLogger(ExternalSMSService.class);
 * 
 * private static final String SMS_RESPONSE_NOT_SUCCESSFUL =
 * "Sms response not successful";
 * 
 * private SmsProperties1 smsProperties; private RestTemplate restTemplate;
 * 
 * 
 ...
 * 
 * 
 * @Value("${sms.verify.ssl:true}") private boolean verifySSL;
 * 
 * 
 ...
 * 
 * 
 * @Autowired public ExternalSMSService(SmsProperties1 smsProperties,
 * RestTemplate restTemplate) {
 * 
 * this.smsProperties = smsProperties; this.restTemplate = restTemplate;
 * 
 * if (!verifySSL) { System.out.println("indide ssl check"); TrustStrategy
 * acceptingTrustStrategy = new TrustStrategy() {
 * 
 * @Override public boolean isTrusted(java.security.cert.X509Certificate[]
 * x509Certificates, String s) { return true; }
 * 
 * };
 * 
 * SSLContext sslContext = null; try { sslContext =
 * org.apache.http.ssl.SSLContexts.custom().loadTrustMaterial(null,
 * acceptingTrustStrategy) .build(); } catch (NoSuchAlgorithmException e) {
 ...
 * new NoopHostnameVerifier()); CloseableHttpClient httpClient =
 ...
 * LOGGER.error(String.format("Sms %s is not valid", sms)); return; }
 * submitToExternalSmsService(sms); }
 * 
 * private void submitToExternalSmsService(Sms sms) { try {
 * 
 * String url = smsProperties.getSmsProviderURL(); ResponseEntity<String>
 * response = new ResponseEntity<String>(HttpStatus.OK); if
 * (requestType.equals("POST")) { System.out.println("in post");
 * HttpEntity<MultiValueMap<String, String>> request = getRequest(sms); response
 * = restTemplate.postForEntity(url, request, String.class);
 * System.out.println(""+response); if
 * (isResponseCodeInKnownErrorCodeList(response)) { throw new
 * RuntimeException(SMS_RESPONSE_NOT_SUCCESSFUL); } } else { //final
 * MultiValueMap<String, String> requestBody =
 * smsProperties.getSmsRequestBody(sms);
 * 
 * System.out.println("url "+url); //String final_url =
 * UriComponentsBuilder.fromHttpUrl(url).queryParams(requestBody).toUriString();
 * String final_url =url+smsProperties.queryParams(sms);
 * 
 * if (dontEncodeURL) { final_url = final_url.replace("%20", " ").replace("%2B",
 * "+"); }
 * 
 * System.out.println("final url "+final_url); String responseString =
 ...
 * LOGGER.error("Error occurred while sending SMS to " + sms.getMobileNumber(),
 * e); throw e; } catch(Exception e) { e.printStackTrace();
 * LOGGER.error("Error occurred while sending SMS to : " +
 * sms.getMobileNumber(), e); throw e; } }
 * 
 * 
